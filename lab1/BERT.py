...
class DistilBertForMaskedLM(DistilBertPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            print(prediction_logits.view(-1, prediction_logits.size(-1)).shape)
...
class DistilBertForSequenceClassification(DistilBertPreTrainedModel):
    ...
    def forward(
        ...
        pooled_output = hidden_state[:, 0]  # (bs, dim)  # CLS
        # pooled_output = hidden_state.mean(dim=1)  # (bs, dim)  # mean pooling
        # pooled_output, _ = hidden_state.max(dim=1)  # (bs, dim)  # max pooling
